...
@section('admin-content')
    ...
    <h1>{{ $gallery->id ? 'Edit' : 'Create' }} Gallery
        @if ($gallery->id)
            <a href="#" class="btn btn-danger float-end delete-gallery-button">Delete Gallery</a>
        @endif
    </h1>
    ...
    <div class="row">
        <div class="col-md mb-3">
            ...
            {!! Form::label('submissions_open', 'Submissions Open', ['class' => 'form-check-label ms-3']) !!} {!! add_help(
        ...
        @if (Settings::get('gallery_submissions_reward_currency'))
            <div class="col-md mb-3">
                ...
                {!! Form::label('currency_enabled', 'Enable Currency Rewards', ['class' => 'form-check-label ms-3']) !!} {!! add_help('Whether or not submissions to this gallery are eligible for rewards of group currency.') !!}
            </div>
        @endif
        <div class="col-md mb-3">
            {!! Form::checkbox('prompt_selection', 1, $gallery->prompt_selection, ['class' => 'form-check-input', 'data-toggle' => 'toggle']) !!}
            {!! Form::label('prompt_selection', 'Prompt Selection', ['class' => 'form-check-label ms-3']) !!} {!! add_help(
    ...
    <div class="row">
        <div class="col-md mb-3">
            {!! Form::label('hide_before_start', 'Hide Before Start Time', ['class' => 'form-check-label ms-3']) !!} {!! add_help('If hidden, the gallery will not be shown on the gallery list before the starting time is reached. A starting time needs to be set. Galleries are always visible after the end time.') !!}<br />
