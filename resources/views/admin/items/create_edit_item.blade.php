...
@section('admin-content')
    ...
    <h1>{{ $item->id ? 'Edit' : 'Create' }} Item
        @if ($item->id)
            <a href="#" class="btn btn-outline-danger float-end delete-item-button">Delete Item</a>
        @endif
    </h1>
    ...
    @if (config('lorekeeper.extensions.item_entry_expansion.extra_fields'))
        <div class="row">
            ...
            <div class="col-md">
                ...
                <div class="row">
                    <div class="col-md mb-3">
                        {!! Form::select('artist_id', $userOptions, $item && $item->artist_id ? $item->artist_id : null, ['class' => 'form-control me-2 selectize', 'placeholder' => 'Select a User']) !!}
                    </div>
                    <div class="col-md mb-3">
                        {!! Form::text('artist_url', $item && $item->artist_url ? $item->artist_url : '', ['class' => 'form-control me-2', 'placeholder' => 'Artist URL']) !!}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-md mb-3">
            ...
            {!! Form::label('allow_transfer', 'Allow User → User Transfer', ['class' => 'form-check-label ms-3']) !!} {!! add_help('If this is off, users will not be able to transfer this item to other users. Non-account-bound items can be account-bound when granted to users directly.') !!}
        </div>
        @if (config('lorekeeper.extensions.item_entry_expansion.extra_fields'))
            <div class="col-md mb-3">
                {!! Form::checkbox('is_released', 1, $item->id ? $item->is_released : 1, ['class' => 'form-check-input', 'data-toggle' => 'toggle']) !!}
                {!! Form::label('is_released', 'Is Released', ['class' => 'form-check-label ms-3']) !!} {!! add_help('If this is off, users will not be able to view information for the item/it will be hidden from view. This is overridden by the item being owned at any point by anyone on the site.') !!}
            </div>
        @endif
        <div class="col-md mb-3">
            {!! Form::checkbox('is_deletable', 1, $item->id ? $item->is_deletable : 1, ['class' => 'form-check-input', 'data-toggle' => 'toggle']) !!}
            {!! Form::label('is_deletable', 'Can Be Deleted', ['class' => 'form-check-label ms-3']) !!} {!! add_help('If this is off, users will not be able to delete this item from user or character inventories.') !!}
        </div>
    </div>
            ...
            <table class="table">
                ...
                @foreach ($item->tags as $tag)
                    <tr>
                        ...
                        <td class="text-end"><a href="{{ url('admin/data/items/tag/' . $item->id . '/' . $tag->tag) }}" class="btn btn-outline-primary">Edit</a></td>
                    </tr>
                @endforeach
            </table>
        @else
