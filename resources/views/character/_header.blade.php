...
<h1 class="mb-0">
    @if (config('lorekeeper.extensions.character_status_badges'))
        ...
        <div class="float-end">
            <span class="btn {{ $character->is_trading ? 'badge-success' : 'badge-danger' }} float-end ms-2" data-bs-toggle="tooltip" title="{{ $character->is_trading ? 'OPEN for sale and trade offers.' : 'CLOSED for sale and trade offers.' }}"><i
                    class="fas fa-comments-dollar"></i></span>
            @if (!$character->is_myo_slot)
                <span class="btn {{ $character->is_gift_writing_allowed == 1 ? 'badge-success' : ($character->is_gift_writing_allowed == 2 ? 'badge-warning text-light' : 'badge-danger') }} float-end ms-2" data-bs-toggle="tooltip"
                ...
                <span class="btn {{ $character->is_gift_art_allowed == 1 ? 'badge-success' : ($character->is_gift_art_allowed == 2 ? 'badge-warning text-light' : 'badge-danger') }} float-end ms-2" data-bs-toggle="tooltip"
    ...
    @if ($character->is_visible && Auth::check() && $character->user_id != Auth::user()->id)
        ...
        <a href="#" class="btn btn-outline-info float-end bookmark-button ms-2" data-id="{{ $bookmark ? $bookmark->id : 0 }}" data-character-id="{{ $character->id }}"><i class="fas fa-bookmark"></i>
            {{ $bookmark ? 'Edit Bookmark' : 'Bookmark' }}</a>
    @endif
    @if (config('lorekeeper.extensions.character_TH_profile_link') && $character->profile->link)
        <a class="btn btn-outline-info float-end" data-character-id="{{ $character->id }}" href="{{ $character->profile->link }}"><i class="fas fa-home"></i> Profile</a>
    @endif
    ...
    @if (!$character->is_myo_slot)
        <i data-bs-toggle="tooltip" title="Click to Copy the Character Code" id="copy" style="font-size: 14px; vertical-align: middle;" class="far fa-copy text-small"></i>
    @endif
</h1>
<div class="mb-3">
