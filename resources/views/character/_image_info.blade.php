        ...
        @if ((isset($image->content_warnings) && !Auth::check()) || (Auth::check() && Auth::user()->settings->content_warning_visibility < 2 && isset($image->content_warnings)))
            <div class="alert alert-danger text-center">
                <span class="float-end">
                    <a href="#" data-bs-dismiss="alert" class="btn-close">
                        <i class="fas fa-times" aria-hidden="true"></i>
                    </a>
                </span>
                <img src="{{ asset('images/content-warning.png') }}" class="mb-1" alt="Content Warning" style="height: 10vh;" />
                <h5>
                    <i class="fa fa-exclamation-triangle me-2"></i>Character Warnings<i class="fa fa-exclamation-triangle ms-2"></i>
                </h5>
        ...
        <div class="card character-bio w-100">
            ...
            <div class="card-body tab-content">
                <div class="text-end mb-1">
                    <div class="badge text-bg-primary">Image #{{ $image->id }}</div>
                </div>
                ...
                <div class="tab-pane fade show active" id="info-{{ $image->id }}">
                    <div class="row g-0">
                        ...
                        <div class="col-lg-8 col-7 ps-1">{!! $image->species_id ? $image->species->displayName : 'None' !!}</div>
                    </div>
                    ...
                    <div class="row g-0">
                        ...
                        <div class="col-lg-8 col-7 ps-1">{!! $image->rarity_id ? $image->rarity->displayName : 'None' !!}</div>
                    </div>
                                    ...
                                    @foreach ($traitgroup as $key => $group)
                                        <div class="mb-2">
                                            ...
                                            @foreach ($group as $feature)
                                                <div class="ms-md-2">{!! $feature->feature->displayName !!} @if ($feature->data)
                                                        ({{ $feature->data }})
                                                    @endif
                                                </div>
                                            @endforeach
                ...
                @if (Auth::check() && Auth::user()->hasPower('manage_characters'))
                    <div class="tab-pane fade" id="settings-{{ $image->id }}">
                        ...
                        <div class="mb-3">
                            ...
                            {!! Form::label('is_visible', 'Is Viewable', ['class' => 'form-check-label ms-3']) !!} {!! add_help('If this is turned off, the image will not be visible by anyone without the Manage Masterlist power.') !!}
                        </div>
                        <div class="mb-3">
