...
@section('profile-content')
    ...
    @if ($character->user_id == Auth::user()->id)
        ...
        @if ($character->is_tradeable || $character->is_sellable)
            <div class="mb-3 disabled">
                ...
                {!! Form::label('is_trading', 'Up For Trade', ['class' => 'form-check-label ms-3']) !!} {!! add_help('This will place the character on the list of characters that are currently up for trade. This does not have any other functionality, but allow users looking for trades to find your character easily.') !!}
            </div>
        @else
    ...
    @if ($character->user_id != Auth::user()->id)
        <div class="mb-3">
            ...
            {!! Form::label('alert_user', 'Notify User', ['class' => 'form-check-label ms-3']) !!} {!! add_help('This will send a notification to the user that their character profile has been edited. A notification will not be sent if the character is not visible.') !!}
        </div>
    @endif
    <div class="text-end">
