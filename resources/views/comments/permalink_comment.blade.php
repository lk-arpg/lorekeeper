...
@section('content')
    ...
    <h5>
        @if (count($comment->children))
            <a href="{{ url('comment/') . '/' . $comment->endOfThread->id }}" class="btn btn-secondary btn-sm me-2">Go To End Of Thread</a>
        @endif
        @if (isset($comment->child_id))
            <a href="{{ url('comment/') . '/' . $comment->child_id }}" class="btn btn-secondary btn-sm me-2">See Parent</a>
            <a href="{{ url('comment/') . '/' . $comment->topComment->id }}" class="btn btn-secondary btn-sm me-2">Go To Top Comment</a>
        @endif
    </h5>
