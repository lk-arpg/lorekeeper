...
<div class="row mb-3">
    ...
    <div class="col">
        ...
        <div class="row g-0">
            <div class="col h2 text-center text-md-start">
                {!! $user->displayName !!}
                ...
                @endif
                <a href="{{ url('reports/new?url=') . $user->url }}"><i class="fas fa-exclamation-triangle fa-xs text-danger" data-bs-toggle="tooltip" title="Click here to report this user." style="opacity: 50%;"></i></a>
            </div>

            @if ($user->settings->is_fto)
                <div class="col-md-1 text-center">
                    <span class="btn text-bg-success float-md-end" data-bs-toggle="tooltip" title="This user has not owned any characters from this world before.">FTO</span>
                </div>
            @endif
        </div>
            ...
            <div class="row g-0 col-sm-5">
                ...
                <div class="col-lg-9 col-md-9 col-8">
                    ...
                    @if (count($aliases) > 1 && config('lorekeeper.extensions.aliases_on_userpage'))
                        <a class="small collapse-toggle collapsed" href="#otherUserAliases" data-bs-toggle="collapse">&nbsp;</a>
                        <p class="collapse mb-0" id="otherUserAliases">
                            @foreach ($aliases as $alias)
                                @if ($alias != $user->primaryAlias)
                                    <a href="{{ $alias->url }}"><i class="{{ $alias->config['icon'] }} fa-fw me-1" data-bs-toggle="tooltip" title="{{ $alias->alias . '@' . $alias->siteDisplayName }}"></i></a>
                                @endif
                            @endforeach
                        </p>
                    @endif
...
<div class="row mb-4 profile-assets">
    <div class="col-md mb-3 mb-md-0">
        <div class="card profile-currencies profile-assets-card h-100">
            <div class="card-body text-center">
                ...
                <div class="text-end"><a href="{{ $user->url . '/bank' }}">View all...</a></div>
            </div>
        </div>
    </div>
    <div class="col-md">
        <div class="card profile-inventory profile-assets-card h-100">
            <div class="card-body text-center">
                ...
                <div class="text-end"><a href="{{ $user->url . '/inventory' }}">View all...</a></div>
            </div>
        </div>
    </div>
</div>
...
<div class="text-end"><a href="{{ $user->url . '/characters' }}">View all...</a></div>
<hr class="mb-5" />
